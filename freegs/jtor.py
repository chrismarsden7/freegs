...
class ConstrainBetapIp(Profile):
    ...
    def Jtor(self, R, Z, psi, psi_bndry=None):
        ...
        # betap = (2mu0) * volume_av(p) / (flux_surf_av(B_poloidal**2))
        #       = - (2L*Beta0*mu0/Raxis) * volume_av(pfunc) / (flux_surf_av(B_poloidal**2))

        p_int = self.eq.calc_volume_averaged(pfunc)
        b_int = self.eq.calc_volume_averaged(self.eq.Bpol(R ,Z)**2.0)
...
class BetapIpConstrainedSplineProfiles(Profile):
    ...
    def __init__(self, eq=None, betap=None, Ip=None, Raxis=None, psi_n=None, pprime=None, ffprime=None, fvac=None):
        ...
        # Create 1D splines for the integral of pprime, ffprime - this will be used in constraining betaP.
